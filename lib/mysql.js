...
MySQL.prototype.getCounts = function (model, props) {
  var aProps;
  if (props === undefined) {
    return '';
  } else if (typeof props === 'string' && props.length > 0) {
    aProps = [];
    aProps.push(props)
  } else if (Array.isArray(props) && props.length > 0) {
    aProps = props;
  } else {
    console.log('SYNTAX ERROR: `count` parameter should be string or array!')
    return '';
  }

  var oModelOpts = this._models[model].settings,
      sCountsQuery = '',
      self = this,
      sFromTable;

  aProps.forEach(function(sProp, iIndex) {
    if (oModelOpts.relations && oModelOpts.relations[sProp]) {

      // hasAndBelongsToMany relation uses table with composite name
      if (oModelOpts.relations[sProp].type === 'hasAndBelongsToMany') {
        sFromTable = model + oModelOpts.relations[sProp].model
      } else {
        sFromTable = oModelOpts.relations[sProp].model
      }
      ...
      sCountsQuery += ', (SELECT COUNT(*) FROM ' + self.tableEscaped(sFromTable) + ' AS inner_query_' + iIndex
        ...
        + ' = '
        + 'inner_query_' + iIndex + '.' + self.columnEscaped(oModelOpts.relations[sProp].model, oModelOpts.relations[sProp].foreignKey) + ') AS ' + sProp + 'Count'
    }
  })
...
MySQL.prototype.all = function all(model, filter, callback) {
  ...
  var sql = 'SELECT ' + this.getColumns(model, filter.fields) + this.getCounts(model, filter.count) + ' FROM ' + this.tableEscaped(model) + ' outer_query';
